...
const Notes = () => {
    ...
    columns: [
      ...
      },
      {
        ...
        valueGetter: ({ row }) => {
          ...
          if (row.carer && row.carerName) return row.carerName;
          if (row.carer || row.carerName)
            return (
              row.carerName || row.carer?.firstName + "" + row.carer?.lastName
            );
          return null;
        },
        renderCell: ({ row }) => {
          if (row.carerId === "") {
            ...
            return (
              ...
              >
                <PriorityHighIcon fontSize="small" color="warning" />{" "}
                {`${row.carer.firstName} ${row.carer?.lastName}`}
              </p>
            );
          } else {
            return (
    ...
    if (posts.posts) {
      setTable((prev) => {
        return {
          ...prev,
          rows: JSON.parse(
            JSON.stringify(posts.posts).replace(/:null/gi, ':""')
          ),
        };
      });
    }
  ...
  const columnVisibility = () => {
    const userSettings = JSON.parse(
      localStorage.getItem("postColumnVisibility")
    );
  ...
  return (
    <div className="w-full max-w-screen-lg mx-auto flex flex-col ">
      <Backdrop open={posts.status === "loading"} className={`z-40`}>
        <CircularProgress />
      </Backdrop>
      <Dialog
