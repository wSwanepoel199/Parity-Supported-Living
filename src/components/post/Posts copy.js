...
const Posts = () => {
  ...
  const { isLoading, isFetching, isSuccess, refetch } = useGetPostsQuery(undefined, { refetchOnMountOrArgChange: true });
  ...
  useEffect(() => {
    ...
    if (mounted.current) {
      if (isSuccess && postState.posts && postState.posts?.length !== table.rows.length) {
      ...
      } else if (parsedPosts && parsedPosts !== JSON.stringify(table.rows)) {
        setTable(prev => {
          return {
            ...prev,
            rows: JSON.parse(parsedPosts)
          };
        });
      }
  ...
  }, [mounted, postState.posts, isSuccess, fullScreen, userState.user.role, table, refetch, isLoading, isFetching,]);
